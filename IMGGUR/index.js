...
getById("get_btn")?.addEventListener("click",async ()=>{
    ...
    data.map((item)=>{
        ...
        // div.style.width=item.width/10
        div.style.minHeight=(item.height/10)+80
        div.innerHTML=`
        <div><img height="${item.height/10}" src=${item.urls?.regular} alt="" "></div>
            <div>
                <p>${item.alt_description||item.description}</p>
            </div>
            <div class="icons" style="max-height=80">
                <div>
